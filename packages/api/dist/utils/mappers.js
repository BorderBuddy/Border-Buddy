...
exports.flightInfoMapper = exports.airlineInfoMapper = void 0;
exports.airlineInfoMapper = (airlines, code) => {
    return airlines.find(airline => airline.iata === code);
};
exports.flightInfoMapper = fsFlightData => {
    const { arrivalCityName, arrivalAirportName, departureCityName, departureAirportName, arrivalUtcOffset, departureUtcOffset, arrivalCountryName, departureCountryName, arrivalAirportFsCode, departureAirportFsCode } = mapAirportInfo(fsFlightData);
    const { airlineName, flightNumber, carrierFsCode } = mapAirlineDetails(fsFlightData);
    const { arrivalTimeLocal, departureTimeLocal, arrivalTimeUtc, departureTimeUtc } = mapTimes(fsFlightData, arrivalUtcOffset, departureUtcOffset);
    return {
        departureCityName,
